...
test('does not require tasa_seguimiento when hace_seguimiento is casi_nunca', () => {
  const answers = fullAnswers({ hace_seguimiento: 'casi_nunca' });
  delete answers.tasa_seguimiento;
  const result = validateSubmission({
    name: 'Ana',
    whatsapp: '+51999999999',
    email: 'ana@example.com',
  ...
  assert.equal(result.ok, true);
  assert.equal(result.data.answers.tasa_seguimiento, undefined);
});
